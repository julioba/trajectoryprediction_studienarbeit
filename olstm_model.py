...
class OLSTMModel(nn.Module):
    ...
    def __init__(self, args, infer=False):
        ...
        # Store required sizes                      PARAMETRO                           VALORES TIPICOS
        self.rnn_size = args.rnn_size               # TAMANO DE LA RED NEURONAL
        self.grid_size = args.grid_size             # DIMENSION DE LA TABLA
        self.embedding_size = args.embedding_size   # DIMENSION DEL EMBEDDING
        self.input_size = args.input_size           # DIMENSION DEL INPUT
        self.output_size = args.output_size         # DIMENSION DEL OUTPUT
        self.maxNumPeds = args.maxNumPeds           # MAXIMO NUMERO DE PEATONES
        self.seq_length = args.seq_length           # LONGITUD DE LA SECUENCIA
        self.gru = args.gru                         #

        # The LSTM cell
        self.cell = nn.LSTMCell(2 * self.embedding_size, self.rnn_size)

        if self.gru: # SI SE QUIERE HACER GRU...
            self.cell = nn.GRUCell(2 * self.embedding_size, self.rnn_size)

        # Linear layer to embed the input position   e
        self.input_embedding_layer = nn.Linear(self.input_size, self.embedding_size)


        # Linear layer to embed the social tensor, which is the map of the size of the grid in a square way
    ...
    def getObsTensor(self, grid):
        ...
        # For each pedestrian, the tensor is the grid
    ...
    def forward(self, *args):
        ...
        grids: Grid masks
        hidden_states: Hidden states of the peds  -- Necessary?
