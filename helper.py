...
def get_all_file_names(path):
    ...
    files = []
    print("We are currently in get_all_file_names.")
    print(path)
    print(walk(path))
...
def sample_validation_data_vanilla(x_seq, Pedlist, args, net, look_up, num_pedlist, dataloader):
    ...
    # Construct variables for hidden and cell states
    hidden_states = Variable(torch.zeros(numx_seq, net.args.rnn_size))#, int(torch.no_grad())) #volatile=True
    if args.use_cuda:
        hidden_states = hidden_states.cuda()
    if not args.gru:
        cell_states = Variable(torch.zeros(numx_seq, net.args.rnn_size))#, torch.no_grad()) #volatile=True
        if args.use_cuda:
            cell_states = cell_states.cuda()
    else:
        cell_states = None

    ret_x_seq = Variable(torch.zeros(args.seq_length, numx_seq, 2))#, torch.no_grad()) #volatile=True
