...
class DataLoader():
    ...
    def __init__(self, f_prefix, batch_size=5, seq_length=20, num_of_validation=0, forcePreProcess=False, infer=False,
        ...
        num_of_validation : number of validation dataset will be used --# CHANGE THIS PARAMETER
        infer : flag for test mode                                    --# CHANGE THIS PARAMETER
    ...
    def get_directory_name_with_pointer(self, pointer_index):
        ...
        folder_prename = self.data_dirs[pointer_index].split('\\')[-2]
        folder_name = folder_prename.split('/')[2]
        return folder_name
    ...
    def write_to_plot_file(self, data, path):
        # write plot file for further visualization in pkl format -- or in txt
        self.reset_batch_pointer()
        for file in range(self.numDatasets):
            file_name = self.get_file_name(file)
            file_name = file_name.split('.')[0]  + '.pkl' #+ '.txt'
